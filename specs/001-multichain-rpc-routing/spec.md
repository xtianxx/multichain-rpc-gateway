...
- **FR-001**: The gateway MUST expose a single unified endpoint for every supported chain; the client addresses the target chain by including its chain id in the request (EIP-695 style).
...
- **FR-014**: Metrics MUST cover request rate, error rate, and latency percentiles per chain and per upstream.
...
- **SC-002**: The gateway's added latency at p50, excluding upstream latency, stays within 20% of the direct-upstream baseline as measured by the included benchmark.
...
- Chain addressing: a single unified endpoint with a chain id parameter (confirmed with user); no per-chain URL paths in v1.
...
- Configuration (chains, upstreams, secrets) comes from environment variables or config files; secrets are never committed to the repository.
- Batch size and request body size limits use a configurable cap with a documented error code (reasonable default for abuse protection).
